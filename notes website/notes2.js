...
// variables
const n_id = []; const n_no = []; const n_desc = []
...
function disp_items () {
  ...
  if (note_id3 != null || note_no3 != null || note_desc3 != null) {
    for (let i = 0; i < note_no3; i++) {
      last.insertAdjacentHTML('beforebegin', `<div class="${note_id3[i]}"><h1 style="font-family: 'Kaushan Script', cursive;
      ">Note:${i + 1}</h1><p style="font-family: 'Kaushan Script', cursive; color:black; text-align: center;  margin:1rem;        background:linear-gradient(170deg,white,white,	rgb(255, 191, 0));


    width:60%;margin:auto;" id="${note_id3[i]}" >${note_desc3[i].desc}</p>
    <button name="${note_id3[i]}" onclick="del_n(this.name)" >Delete Note${i + 1}</button>
    <button name="${note_id3[i]}" onclick="edit_n(this.name)"   >Edit Note${i + 1} </button>
...
add_btn.addEventListener('click', () => {
  ...
  //   last.insertAdjacentHTML('afterend', `<div id="note${N}"><h1>note:${N}</h1><p style="text-align:centre;  margin:1rem;  color:white;   background-color: grey;
  // width:60%;margin:auto;">${document.getElementById('write_area').value}</p>
  // <button onclick=del_n() class="del_n">Delete</button>
  // <button id="edit_n">Edit</button>
  // </div>`)
...
function edit_n(ele) {
  ...
  document.getElementById(`${ele}`).insertAdjacentHTML('afterend', `<button name="${ele}" onclick="change_n(this.name)">Overwrite</button>`)
